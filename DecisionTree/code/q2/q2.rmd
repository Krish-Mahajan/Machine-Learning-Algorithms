...
g<-ggpairs(data[c("Non.Flavanoid.Phenols","Flavanoids","Proline","color.intensity")],title="Pairwise plot of 4 most correlated features")


#plotting pair wise plot least correlated features
g<-ggpairs(data[c("Alcalinity.of.ash","Proline","color.intensity","Hue","OD280")],title="Pairwise plot of 4 least correlated features")
```  
...
#Function for 0-1 normalization
f2c1 <- function(x) {
(x - min(x, na.rm=TRUE))/(max(x,na.rm=TRUE) -min(x, na.rm=TRUE))
}
norm_data <- as.data.frame(lapply(data[,-14], f2c1))
...
f2c2<-function(data){
  ...
  std_data<-scale(data[,-14])
  std_data<-cbind(norm_data,data$class)
  colnames(norm_data)[14]<-'class'
  std_data<-as.data.frame(norm_data)
  return (std_data)
  } 
